...
$path = 'C:\Users\Username\LocalPowerShellModules\'
#
#
# Step 4: 
#
...
Function Is-Role-4-Hour($tmpRole) {
    if ($tmpRole -eq 2 -or $tmpRole -eq 4) {
        return $true
    }
    return $false
}
...
Function Initiate-Renewal($all) {
    if ($all) {
        if ($toastNotification) {
            New-BurntToastNotification -AppLogo $yellowImagePath -Text "All roles expiring...", 'Press R key on console to renew'
        }
        ...
        if ($toastNotification) {
            New-BurntToastNotification -AppLogo $yellowImagePath -Text "1 hour roles expiring...", 'Press R key on console to renew'
        }
